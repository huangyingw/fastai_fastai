...
#
# - [`_affine_grid`](https://github.com/fastai/fastai/blob/master/fastai/vision/image.py#L513) is reponsible for creating the grid of coordinates
# - [`_affine_mult`](https://github.com/fastai/fastai/blob/master/fastai/vision/image.py#L523) is in charge of doing the affine multiplication on that grid
# - [`_grid_sample`](https://github.com/fastai/fastai/blob/master/fastai/vision/image.py#L500)is the function that is responsible for the interpolation step
...
# In-place version of [`logit`](/torch_core.html#logit).
...
# Show the image on `ax` with `figsize`, optional `title`, `hide_axis` will hide the axis, `cmap` is used and if `y` is passed, it is showed on the same `ax`.
